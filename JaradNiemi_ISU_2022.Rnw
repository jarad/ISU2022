...
We assume a Gaussian Process  
\[ f \sim \mathcal{GP}\left(m, k \right) \implies 
Y = (Y_1,\ldots,Y_N)^\top \sim \mathcal{N}(m_x, \Sigma) \]
\pause
...
= \mbox{argmax}_{\theta} \, \mathcal{N}\left(y; 0, \Sigma(\theta)\right)
\]
...
\item Optimizes the knot locations according to the approximate likelihood or variational objective function
...
Assume $f$ is a zero-mean GP with
...
We'll refer to this as \alert{fiGP} and 
\alert{dynamic automatic relevance determination}.
...
\begin{frame}
  ...
  \item fiGP/DARD model for functional inputs
  \end{itemize}
